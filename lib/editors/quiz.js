(function(Form) {
   ...
   *
   * Special options:
   * @param {String} [options.schema.itemType]          The editor type for each item in the quiz. Default: 'Text'
   ...
   */
  Form.editors.Quiz = Form.editors.Base.extend({

    events: {
      'click [data-action="next"]': function(event) {
    ...
    initialize: function(options) {
      ...
      this.Editor = (function() {
        ...
        //Use Quiz-specific version if available
        if (editors.Quiz[type]) return editors.Quiz[type];
    ...
    render: function() {
      ...
      //Store a reference to the question list (questions container)
      this.$list = $el.is('[data-questions]') ? $el : $el.find('[data-questions]');

      //Add existing items
      if (value.length) {
        _.each(value, function(itemValue) {
          self.addQuestion(itemValue);
        });
      }

      //If no existing items create an empty one, unless the editor specifies otherwise
      else {
        if (!this.Editor.isAsync) this.addQuestion();
      }
     ...
     */
    addQuestion: function(value, userInitiated) {
      var self = this,
          editors = Form.editors;

      //Create the item
      var question = new editors.Quiz.Question({
        list: this,
        form: this.form,
        schema: this.schema,
        question: value,
      ...
      var _addQuestion = function() {
        ...
        self.$list.append(question.el);
        
        
        if (userInitiated || value) {
          question.addEventTriggered = true;
        }
        
        if (userInitiated) {
          self.trigger('add', self, question.editor);
      ...
      if (this.Editor.isAsync) {
        //question.editor.on('readyToAdd', _addQuestion, this);
      }
    ...
    saveAnswer: function(){
      // send ajax to save an answer on server
    },

    /**
     * Remove an question from the list
     * @param {Quiz.Question} question
     */
    removeItem: function(item) {
      ...
      var index = _.indexOf(this.questions, item);
      ...
      if (!this.questions.length && !this.Editor.isAsync) this.addQuestion();
    },
    ...
    template: _.template('\
      <div>\
        ...
        <button type="button" data-action="next">Далее</button>\
      </div>\
  ...
  /**
   * A single question in the list
   *
   * @param {editors.Quiz} options.list The Quiz editor instance this item belongs to
   * @param {Function} options.Editor   Editor constructor function
   * @param {String} options.key        Model key
   * @param {Mixed} options.value       Value
   * @param {Object} options.schema     Field schema
   */
  Form.editors.Quiz.Question = Form.editors.Base.extend({

    events: {
      'keydown input[type=text]': function(event) {
        if(event.keyCode !== 13) return;
        event.preventDefault();
        this.list.addQuestion();
    ...
    initialize: function(options) {
      ...
      this.answerSchema = options.question.answerSchema;
    ...
    render: function() {
      ...
      var $el = $($.trim(this.template()));
      //Store a reference to the question list (questions container)
      this.$qTitle = $el.is('[question-title]') ? $el : $el.find('[question-title]');

      this.$qTitle.append(this.question.question)

      this.$list = $el.is('[data-answers]') ? $el : $el.find('[data-answers]');

      //$el.find('[data-answers]').append(this.editor.el);
      
      //Add answers 
      if (answers.length) {
        _.each(answers, function(itemValue) {
          self.addAnswer(itemValue);
        });
      }
  ...
  /**
   * Base modal object editor for use with the Quiz editor; used by Object 
   * and NestedModal list types
   */
  Form.editors.Quiz.Modal = Form.editors.Base.extend({
    ...
    initialize: function(options) {
      ...
      //Dependencies
      if (!Form.editors.Quiz.Modal.ModalAdapter) throw 'A ModalAdapter is required';
    ...
    openEditor: function() {
      ...
      var modal = this.modal = new Form.editors.Quiz.Modal.ModalAdapter({
        content: form,
        animate: true
      });
  ...
  Form.editors.Quiz.Object = Form.editors.Quiz.Modal.extend({
    initialize: function () {
      Form.editors.Quiz.Modal.prototype.initialize.apply(this, arguments);
  ...
  Form.editors.Quiz.NestedModel = Form.editors.Quiz.Modal.extend({
    initialize: function() {
      Form.editors.Quiz.Modal.prototype.initialize.apply(this, arguments);
